...
import { loginAPI, resendOtpAPI, verifyAPI } from "@/app/lib/api/auth";
...
const OtpModal = ({
  ...
  const handleSubmit = async (otpValue?: string) => {
    ...
    try {
      const verifyRes = await verifyAPI({ otp: { code: finalOtp } });
      ...
      if (
        ...
        loginSuccess(
          {
            ...
            loginMethod: loginRes.user.loginMethod || "Email",
        ...
        toast.success(`Chào mừng ${loginRes.user.name} đến với NovelNest!`);
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast.error(
        (err as Error)?.message || "Đã có lỗi xảy ra khi gửi lại mã OTP.",
      );
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm sm:max-w-md overflow-hidden">
        <div className="p-4 sm:p-6">
          ...
          {error && (
            <div className="mb-4">
              <p className="text-sm text-red-600 bg-red-50 py-2 px-3 rounded-lg text-center">
                {error}
              </p>
            </div>
          )}

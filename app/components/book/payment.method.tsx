      ...
      if (userId && orderCode) {
        try {
          ...
          if (!paymentSuccessUrl) {
            toast.error(
              "Lỗi cấu hình: Không tìm thấy URL thanh toán thành công",
            );
            return;
          }
          ...
          if (response.ok && result.success) {
            ...
            setMessage(
              result.message || "Có lỗi xảy ra khi cập nhật thông tin mua sách",
            );
            toast.error(
              result.message || "Có lỗi xảy ra khi cập nhật thông tin mua sách",
            );
    ...
    try {
      ...
      if (fromCart) {
        ...
        for (const item of items) {
          if (!item.bookId || !item.title || !item.price) {
            toast.error(
              `Thông tin sản phẩm "${item.title || "không xác định"}" không đầy đủ`,
            );
            setIsCreatingLink(false);
            return;
          }
    ...
    <div className="min-h-screen bg-[#FFFFFF] dark:bg-[#191B24] py-8 px-4">
      ...
      <div className="max-w-md mx-auto">
        <div className="flex items-center justify-center text-center mb-3">
          <h1 className="text-xl font-semibold text-slate-900 dark:text-gray-200">
            Tóm tắt đơn hàng
          </h1>
        </div>
        ...
        <div className="bg-white rounded-md shadow-sm border border-slate-200/60 overflow-hidden mb-6">
          ...
          <div
            className={`mx-6 mb-6 p-4 rounded-xl border-2 transition-all duration-200 cursor-pointer ${
